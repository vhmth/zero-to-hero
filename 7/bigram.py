...
# when running on lambdalabs instance, for some reason, there's a device mismatch
# between tensors. this is probably a legitimate bug, but I'm just doing this to
# get it to run
if device == 'cuda':
...
class FeedForward(nn.Module):
    ...
    def __init__(self, n_embd):
        ...
        self.net = nn.Sequential(
            nn.Linear(n_embd, 4 * n_embd), # inner layer of ffwd should be 4x the channel size of the projection
            nn.ReLU(),
...
class Block(nn.Module):
    ...
    def forward(self, x):
        # add x to create a residual pathway
